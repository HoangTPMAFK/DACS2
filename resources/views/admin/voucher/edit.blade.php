                        ...
                        <div class="bg-white rounded-b-lg shadow-lg border-b border-slate-300">
                            <div class="flex flex-col md:flex-row flex-wrap justify-around p-4 bg-white rounded-lg shadow-lg text-lg mb-4">
                                <div class="md:w-1/2 mb-4">
                                    <input type="checkbox" name="allProduct" id="allProduct" @if($voucher['allProduct']) {{"checked"}} @endif onchange="toggleProductList()">
                                    <label for="allProduct">Áp dụng cho mọi sản phẩm</label>
                                </div>
                                <div class="md:w-1/2 mb-4">
                                    <span>Lượng giảm</span>
                                    <input class="p-2 border border-slate-300" type="number" name="discountAmount" id="discountAmount" value="{{$voucher['discountAmount']}}">
                                </div>
                                <div class="md:w-1/2 mb-4">
                                    <span>Đơn vị</span>
                                    <select name="unit" id="unit" class="p-2 border border-slate-300" value="{{$voucher['unit']}}">
                                        <option value="%">%</option>
                                ...
                                <div class="md:w-1/2 mb-4">
                                    ...
                                    <input class="p-2 border border-slate-300" type="number" name="maxDiscount" id="maxDiscount" value="{{$voucher['maxDiscount']}}">
                                </div>
                                        ...
                                        <div class="md:w-1/2 flex flex-row justify-between items-center">
                                            <div class="flex flex-row gap-4">
                                                ...
                                                <div class="flex flex-col">
                                                    <div>{{$product['product_name']}} @if(!is_null($product['edition'])) {{"- ".$product['edition']}} @endif @if(!is_null($product['color'])) {{"- ".$product['color']}} @endif </div>
                            ...
                            <div class="mb-2 text-right">
                                <button class="bg-blue-600 p-2 text-lg text-white mr-2 rounded-lg shadow-lg">Cập nhật mã giảm giá</button>
                            </div>
                        </div>
                    </div>
                </form>
